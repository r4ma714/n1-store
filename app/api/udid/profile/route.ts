...
export async function GET(req: NextRequest) {
  ...
  const callbackUrl = new URL(
    `/api/udid/callback?token=${encodeURIComponent(token)}`,
    req.nextUrl.origin
  ).toString();
...
<dict>
  ...
  <string>This temporary profile asks your iPhone or iPad for UDID so N1 Store can register your device.</string>
  <key>PayloadType</key>
